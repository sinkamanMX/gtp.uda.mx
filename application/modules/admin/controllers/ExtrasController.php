...
class admin_extrasController extends My_Controller_Action
{
	protected $_clase = 'mextras';
    ...
    public function indexAction(){
    	try{
			...
			$classObject = new My_Model_Extras(); 
			$this->view->datatTable = $classObject->getRowsEmp(); //id de empresa
		} catch (Zend_Exception $e) {
            echo "Caught exception: " . get_class($e) . "\n";
        	echo "Message: " . $e->getMessage() . "\n";                
        }  
		...
		if($this->operation=='update'){			
			if($this->idToUpdate>-1){
				 ...
				 if($updated['status']){
				 	...
				 	$this->_redirect('/admin/extras/index');
				 }
			}else{
				$this->errors['status'] = 'no-info';
			}	
