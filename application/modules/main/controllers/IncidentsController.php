...
class main_incidentsController extends My_Controller_Action
{
	protected $_clase = 'incidents';
    ...
    public function getinfoAction(){
		...
		if($this->operation=='update'){			
			if($this->idToUpdate>-1){
				 ...
				 if($updated['status']){
				 	...
				 	$this->_redirect('/main/incidents/index');
				 }
			}else{
				$this->errors['status'] = 'no-info';
			}	
		...
		$this->view->prioridad  = $functions->cboPrioridad(@$dataInfo['PRIORIDAD']);
		$this->view->enviarmail = $functions->cboSiNo(@$dataInfo['CORREO']);
