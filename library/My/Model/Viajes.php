...
class My_Model_Viajes extends My_Db_Table
{
    ...
    public function getDataExport($idObject){
		$result= Array();
		$this->query("SET NAMES utf8",false); 
    	$sql ="SELECT V.ID_VIAJE, V.CLAVE, V.INICIO, V.FIN, S.DESCRIPCION AS SUCURSAL, U.ECONOMICO,C.NOMBRE AS CLIENTE, COUNT(I.ID_VIAJE) AS INCIDENCIAS, O.NOMBRE, 
    			T.DESCRIPCION AS TRANSPORTISTA, V.DESCRIPCION AS NDESC, E.DESCRIPCION AS DESC_E
				FROM GTP_VIAJES V
				INNER JOIN SUCURSALES S ON V.ID_SUCURSAL = S.ID_SUCURSAL
				INNER JOIN GTP_ESTATUS_VIAJE ST ON V.ID_ESTATUS = ST.ID_ESTATUS
				INNER JOIN GTP_UNIDADES U ON V.ID_UNIDAD = U.ID_UNIDAD
				...
				LEFT JOIN GTP_INCIDENCIAS_VIAJE I ON V.ID_VIAJE = I.ID_VIAJE
    ...
    public function getDataReport($idObject){
		$result= Array();
		$this->query("SET NAMES utf8",false); 
    	$sql ="SELECT V.ID_VIAJE, V.CLAVE, V.INICIO, V.FIN, S.DESCRIPCION AS SUCURSAL, U.ECONOMICO,C.NOMBRE AS CLIENTE, COUNT(I.ID_VIAJE) AS INCIDENCIAS, O.NOMBRE, T.DESCRIPCION AS TRANSPORTISTA
				FROM GTP_VIAJES V
				INNER JOIN SUCURSALES S ON V.ID_SUCURSAL = S.ID_SUCURSAL
				INNER JOIN GTP_ESTATUS_VIAJE ST ON V.ID_ESTATUS = ST.ID_ESTATUS
				INNER JOIN GTP_UNIDADES U ON V.ID_UNIDAD = U.ID_UNIDAD
